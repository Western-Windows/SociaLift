...
class EnhancementState(TypedDict):
    ...
    holidays_trends: List[Dict[str, Any]]
...
def _persona_block(persona: Dict[str, Any]) -> str:
    return (
        ...
        f"- Tone: {persona.get('emotional_tone')}\n"
        f"- Keywords: {', '.join(persona.get('keywords', []))}\n"
        f"- Voice: {persona.get('voice_description')}"
    )
...
def generate_from_idea(state: EnhancementState) -> dict:
    ...
    comp_text = "\n".join(
        f"- {c.get('context', '')[:200]}"
        for c in state["competitors_data"][:3] if c.get("context")
    ) or "No competitor data."
    holiday_text = "\n".join(
        f"- {h.get('holiday_name')} on {h.get('date')}"
        for h in state["holidays_trends"][:2]
    ) or "No upcoming holidays."
...
**Upcoming holidays/trends:**
{holiday_text}

Write an engaging social media post that matches the persona. Output only the post text.
"""
    response = llm.invoke([
...
if __name__ == "__main__":
    ...
    initial_state: EnhancementState = {
        ...
        "holidays_trends": [
            {"holiday_name": "Eid El Fitr", "date": "2025-03-30"}
        ],
        "mode": None,
        "input_type": None,
        "raw_input": None,
