#!/usr/bin/env python3
"""
Regional Trends & Holidays Fetcher
1. Detects Page Country (Defaults to Egypt).
2. Fetches Upcoming Public Holidays for that Country.
3. Fetches Real-time Google Trends for that Country.
4. Exports actionable JSON.
"""
...
class RegionalManager:
    def __init__(self):
        ...
        # Mapping Facebook Country Names -> ISO Codes (for Holidays lib)
        self.iso_map = {
        ...
        # Mapping ISO Codes -> Google Trends 'pn' parameter (lowercased, underscored)
        self.trends_map = {
    ...
    def get_page_region(self):
        ...
        try:
            ...
            # Logic: Check map, fallback to EG
    ...
    def get_upcoming_holidays(self, iso_code):
        ...
        try:
            ...
            # Load the country specific calendar
            country_holidays = holidays.country_holidays(iso_code, years=year)
            
            # 2. Filter for upcoming (Next 60 days)
            upcoming = []
            ...
            for date, name in sorted(country_holidays.items()):
                if today <= date <= limit_date:
                    ...
                    # Content advice based on proximity
                    advice = "Plan content."
    ...
    def get_regional_trends(self, iso_code):
        ...
        """
        trend_location = self.trends_map.get(iso_code, 'egypt') # Default to egypt trends
    ...
    def save_to_json(self, data, filename="regional_events_holidays.json"):
        try:
